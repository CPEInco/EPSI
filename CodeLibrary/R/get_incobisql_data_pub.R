...
get_InCo_data <- function(author,team,platform,solution,ids,username,password, names=NULL, start = format(Sys.Date()-1,format = "%Y-%m-%d"), end = format(Sys.Date(),format = "%Y-%m-%d"), timeZone = "UTC", forecasts = c("a","l","d","l1","f")[2], DA_days = -1L, DA_hour = 12L, hours_add = 0L, latest = F, tidy = F, CurveIdStr = "CurveId", ForecastDateStr="ForecastDateUTC", ValueDateStr="ValueDateUTC", tableStr="Timeseries1_v02", startF = NULL, endF = NULL, ForecastDate=F) {
  ...
  Database = "InCo"
  sqlpath_latest <- "CodeLibrary/SQL Functions/Pub/latest_FC.sql"
  sqlpath_first <- "CodeLibrary/SQL Functions/Pub/first_FC.sql"
  sqlpath_latest1 <- "CodeLibrary/SQL Functions/Pub/latest1_FC.sql"
  sqlpath_dah <- "CodeLibrary/SQL Functions/Pub/dah_FC.sql"
  sqlpath_a <- "CodeLibrary/SQL Functions/Pub/simple.sql"
  ids <- as.integer(ids)
