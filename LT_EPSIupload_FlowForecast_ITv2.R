...
get_flowforecast <- function(forecastDateTime, startDateTime, endDateTime){
  ...
  priceareas <- c("IT_CNOR", "IT_NOR", "IT_SUD", "IT_CSUD")
  ...
  getData <- dplyr::bind_rows(getDatameteo,getDatats1) %>% 
    ...
    dplyr::filter(Area %in% c("aNOR","CNOR","SUD","CSUD"))
 ...
 # 
  ...
  for(i in(1:length(loopdates))){
    ...
    idatajoin <- iActualData %>% 
      ...
      dplyr::left_join(iFundementalData, by = c("ValueDate" = "ValueDate", "name2" = "Area", "ValueDateCET" = "ValueDateCET", "ValueDateDay" = "ValueDateDay"),suffix=c(".NOR",".CNOR")) %>% 
      dplyr::left_join(filter(iFundementalData,Area=="CSUD"), by = c("ValueDate" = "ValueDate", "ValueDateCET" = "ValueDateCET", "ValueDateDay" = "ValueDateDay")) %>%
      dplyr::left_join(filter(iFundementalData,Area=="SUD"), by = c("ValueDate" = "ValueDate", "ValueDateCET" = "ValueDateCET", "ValueDateDay" = "ValueDateDay"),suffix=c(".CSUD",".SUD")) %>%
      dplyr::left_join(todayforecast, by = c("hh" = "hh", "name1" = "Area")) %>% 
      dplyr::left_join(todayforecast, by = c("hh" = "hh", "name2" = "Area"),suffix=c(".NOR_F",".CNOR_F")) %>%
      dplyr::left_join(filter(todayforecast,Area=="CSUD"), by = c("hh" = "hh" )) %>% 
      dplyr::left_join(filter(todayforecast, Area=="SUD"), by = c("hh" = "hh"),suffix=c(".CSUD_F",".SUD_F")) %>%
      dplyr::mutate(error_rs_NOR = abs(residualload.NOR - residualload.NOR_F)/demand.NOR_F, 
                    error_d_NOR = abs(demand.NOR - demand.NOR_F)/demand.NOR_F,
                    error_rs_CNOR = abs(residualload.CNOR - residualload.CNOR_F)/demand.CNOR_F,
                    error_d_CNOR = abs(demand.CNOR - demand.CNOR_F)/demand.CNOR_F,
                    error_rs_CSUD = abs(residualload.CSUD - residualload.CSUD_F)/demand.CSUD_F, 
                    error_d_CSUD = abs(demand.CSUD - demand.CSUD_F)/demand.CSUD_F,
                    error_rs_SUD = abs(residualload.SUD - residualload.SUD_F)/demand.SUD_F,
                    error_d_SUD = abs(demand.SUD - demand.SUD_F)/demand.SUD_F,
                    total_error = error_rs_NOR + error_d_NOR + error_rs_CNOR + error_d_CNOR + error_rs_CSUD + error_d_CSUD + error_rs_SUD + error_d_SUD + 0.005*lubridate::day(lubridate::days(idate - ValueDateDay))
      ) %>% 
...
for (j in 28:1) {
forecastDateTime <- lubridate::with_tz(as.POSIXct(now(), tz = "Europe/Paris")-lubridate::days(j+1)-lubridate::hours(5),"UTC")
startDateTime <-lubridate::with_tz(as.POSIXct(lubridate::ceiling_date(lubridate::now("Europe/Paris")-lubridate::days(j+1),unit="days"), tz = "Europe/Paris"),"UTC")
endDateTime <- lubridate::with_tz(as.POSIXct(lubridate::ceiling_date(lubridate::now("Europe/Paris")-lubridate::days(j),unit="days"), tz = "Europe/Paris"),"UTC")

df_10<-get_flowforecast(forecastDateTime, startDateTime, endDateTime) %>%
  dplyr::rename("137610_hour"="NOR_CNOR_Flow")

df_10<-df_10[order(df_10$ValueDate),]
...
dfmani_10<-rbind(dfmani_10,df_10)
}

my_data_old_10<-dfmani_Old_10 %>% distinct(`ValueDate`, .keep_all = TRUE)
colnames(my_data_old_10)[2] <- "Forecast_Old_10"
mergedData<-cbind(mergedData,my_data_old_10)
MAPE(mergedData$Forecast_Old_10,mergedData$Value)

inputsqlpath <- "//VMWINFIL01/INCOMAS/Trading/Forward power/Tools/EPSI/upload/sql/ITFlow.sql"
channel<-odbcDriverConnect("DRIVER={ODBC Driver 17 for SQL Server};SERVER=inco-bisql;Database=InCo;uid=sa_ForwardPowerTrading;pwd=sVXTFQrQS3p8")

ITFlow <- sqlQuery(channel,"SELECT TOP (1000) [Auction]
      ,[ValueDateTimeUTC]
      ,[PowerPriceAreaFromCode]
      ,[PowerPriceAreaToCode]
      ,[Value]*-1 Value
      ,[UnitCode]
  FROM [pub].[GME_Transiti]
  where PowerPriceAreaFromCode='IT-CNOR' and PowerPriceAreaToCode='IT-NOR' and ValueDateTimeUTC>='2022-05-31 22:00:00' and ValueDateTimeUTC<='2022-06-28 21:00:00'
  order by ValueDateTimeUTC asc")

ITFlow<-ITFlow %>% 
  dplyr::transmute(ValueDate=as.POSIXct(ValueDateTimeUTC, tz = "UTC"),
                   Value=Value) %>% 
  dplyr::left_join(dfmani)
